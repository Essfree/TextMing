...
public class LdaModel {
	...
	public void saveIteratedModel(int iters, Documents docSet) throws IOException {
		...
		int topNum = 80; //Find the top 20 topic words in each topic
		for(int i = 0; i < K; i++){
		...
		writer = new BufferedWriter(new FileWriter(resPath + modelName + ".keywords"));
		int topNums = 80; //Find the top 20 topic words in each topic
		for(int i = 0; i < K; i++){
			List<Integer> tWordsIndexArray = new ArrayList<Integer>(); 
			for(int j = 0; j < V; j++){
				tWordsIndexArray.add(new Integer(j));
