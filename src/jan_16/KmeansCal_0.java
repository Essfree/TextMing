...
public class KmeansCal_0 {

	public KmeansCal_0(Map<String, Map<String, Double>> tfidfMap, int k) throws IOException {
		Map<String, Integer> fi = Process(tfidfMap,k);
		String desDir="E:\\MUC\\1.16\\Result\\Result.txt";	
...
//		System.out.println(fi.size());
        ...
        for(Iterator<Map.Entry<String,Integer>> it = fiSet.iterator(); it.hasNext(); ){  
            ...
            for(int i = 0;i<6;i++){
            	if(me.getValue()==i)
            		resWriter.append( me.getValue() +" "+me.getKey().substring(me.getKey().lastIndexOf("c"),me.getKey().lastIndexOf(".txt")) + "\n");  
            } 
        }
        resWriter.flush();  
        resWriter.close();  
	    ...
	    while(true){  
            ...
            for(int i = 0; i < snLength; i++){  
                for(int j = 0; j < k; j++){  
                    ...
                    System.out.print("the distance is "+distance[i][j]+" ");
                }  
                System.out.println();
            } 
//          找到与每个点最近的聚类中心
